...
exports.createTask = async (req, res) => {
  try {
    ...
    // 📢 Send notification to contractor's users
    const contractorUsers = await db.users.findAll({
      where: {
        contractor_id: contractor_id,
        user_type: "contractor",
        pushToken: { [Op.ne]: null }, // not null
      },
    });
    for (const contractorUser of contractorUsers) {
      await sendPushNotification(
        ...
        "New Task Assigned",
        `You have a new task: ${task.task_type} Customer: ${task.account_number}`,
        task
      );
    }
...
exports.updateTaskStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Task status updated to ${status}`, task, note });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: error.message });
  }
...
exports.completeTask = async (req, res) => {
  ...
  try {
    ...
    // Deduct cable stock
    if (cable_type && cable_length) {
      const stock = await CableStock.findOne({
        where: { contractor_id, cable_type },
      });
      stock.quantity -= cable_length;
      await stock.save();
      const stockAssignment = await StockAssignment.findOne({
        where: { contractor_id, item_type: cable_type, status: "OPEN" },
      });
    ...
    if (no_atb > 0) {
      ...
      await atbStock.save();
      const stockAssignment = await StockAssignment.findOne({
        where: { contractor_id, item_type: "ATB", status: "OPEN" },
      });
    ...
    if (patchCode > 0) {
      ...
      const stockAssignment = await StockAssignment.findOne({
        where: { contractor_id, item_type: "Patch Cord", status: "OPEN" },
      });
...
exports.approveTask = async (req, res) => {
  try {
    ...
    if (
      task_completion.status === "Accepted" ||
      task_completion.status === "Rejected"
    ) {
      return res
        .status(400)
        .json({ message: "Task completion is not in progress" });
    }
    ...
    res
      .status(200)
      .json({ message: `Task completion status updated to ${status}` });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: error.message });
  }
